export default class Selector {
  /**
   * Choose the correct text based on the number and locale.
   */
  choose(text: string, number: number, locale?: string) {
    ...
    return segments[pluralIndex].trim();
  }

  private extract(segments: string[], number: number) {
    let text;

    for (const part of segments) {
      if (!text) {
        text = this.extractFromString(part, number);
      }
    }
  ...
  private extractFromString(part: string, number: number) {
    ...
    return parseInt(condition) === number ? value : undefined;
  }

  private stripConditions(segments: string[]) {
    return segments.map((part) => part.replace(/^[\{\[]([^\[\]\{\}]*)[\}\]]/, ''));
  }

  private getPluralIndex(number: number, locale?: string) {
    switch (locale) {
      case 'az':
      case 'az_AZ':
      case 'bo':
